...
import {Platform, NavController, Slides, ModalController} from 'ionic-angular';
...
export class MapPage {
  ...
  ionViewWillEnter(){
    ...
    this.proptype = this.locations.type;
    this.proptypenumber = this.locations.proptypenumber;
    this.loadmap();
  }
  ...
  OpenSettings(){
    ...
    modal2.onDidDismiss(data => {
      if(data){
        this.userdata.getUserPrefs().then(data => {
          this.locations.load(this.proptype, data[0].radius, data[0].sort,data[0].scale, data[0].openonly, this.proptypenumber)
            .then(data => {
